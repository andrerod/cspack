...
_.extend(exports, {
  ...
  */
  mkdirRecursive: function (directoryPath, callback) {
    function execute(directoryPath, root, callback) {
      ...
      fs.mkdir(root, function (err1) {
        if (err1) {
          fs.stat(root, function (err2, stat) {
            ...
            execute(dirs.join(path.sep), root, callback);
          });
        } else {
          execute(dirs.join(path.sep), root, callback);
        }
    ...
    execute(directoryPath, undefined, callback);
  },
  ...
  */
  calculateFileHashsum: function (filename, hashingAlgorithm, callback) {
