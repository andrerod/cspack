...
exports.mkdir = function (directoryPath, root) {
...
exports.copyFile = function (source, target, options, cb) {
  var cbCalled = false;
  ...
  if (options) {
    ...
    if (options.createBasepath) {
      exports.mkdir(path.dirname(source));
    }
  }
...
exports.filesChecksum = function (outputRoot, files, callback, processed) {
  if (files.length > 0) {
    var file = files.pop();
...
exports.fileChecksum = function (filename, callback) {
  var algo = 'sha256';
...
exports.addBOM = function (content) {
  return '\ufeff' + content;
};

exports.processTemplate = function (templatePath, templateData, addBOM) {
