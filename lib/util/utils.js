...
_.extend(exports, {
  ...
  */
<<<<<<< HEAD
  calculateChecksum: function (origin, hashingAlgorithm, callback) {
    var hashsum = crypto.createHash(hashingAlgorithm.toLowerCase());
=======
...
>>>>>>> dev2
    ...
    if (origin.filePath) {
      ...
      readStream.on('data', function(chunk) { hashsum.update(chunk); });
      readStream.on('end', function() {
        callback(null, hashsum.digest('base64'));
      });
    } else {
      callback(null, hashsum.update(origin.content).digest('base64'));
    }
  },
  ...
  * @param {function(err, object)} callback The callback function.
  */
  parseXmlString: function (content, callback) {
