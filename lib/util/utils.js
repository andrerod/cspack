...
_.extend(exports, {
  ...
  */
  calculateFileHashsum: function (origin, hashingAlgorithm, callback) {
    var shasum = crypto.createHash(hashingAlgorithm.toLowerCase());

    if (origin.filePath) {
      var readStream = fs.ReadStream(origin.filePath);
      readStream.on('data', function(chunk) { shasum.update(chunk); });
      readStream.on('end', function() {
        callback(null, shasum.digest('base64'));
      });
    } else {
      callback(null, shasum.update(origin.content).digest('base64'));
    }
  },
  ...
  */
  ...
  /**
  * Retrieves the user home directoy.
  *
  * @return {string} The user home directory.
  */
