...
_.extend(HashedPackage.prototype, {
  ...
  * @param {string} origin          The full path to the file to add.
  ...
  */
  addContentDefinition: function (store, name, origin, callback) {
    ...
    utils.calculateChecksum(origin, self.hashingAlgorithm, function (err, checksum) {
      if (err) { return callback(err); }

      var contentDefinition;
      if (store === constants.PackagePaths.LocalContent) {
        // Local content can be reused
        contentDefinition = self.getContentDefinitionByChecksum(checksum);
      }
  ...
  */
  getContentDefinitionByChecksum: function (checksum) {
    var self = this;
