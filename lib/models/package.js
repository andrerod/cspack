...
Package.prototype.addContentDefinition = function (store, filePath, originalFullPath) {
  ...
  if (_.isObject(filePath)) {
    var realFilePath = ('/' + filePath.file).replace(/\//g, '\\');
    var contentDefinition = this.contents.filter(function (content) {
      return content.filePath === realFilePath;
    })[0];

    contentDefinition = _.clone(contentDefinition);
    contentDefinition.filePath = ('/' + filePath.link).replace(/\//g, '\\');
  } else {
...
Package.getLayout = function (type) {
  ...
  case constants.RoleTypes.Worker:
    return [
      ...
      {
        file: 'base/Microsoft.WindowsAzure.ServiceRuntime.dll',
        link: 'base/x64/Microsoft.WindowsAzure.ServiceRuntime.dll'
      },
      ...
      {
        file: 'base/x64/WaHostBootstrapper.exe.config',
        link: 'base/x86/WaHostBootstrapper.exe.config'
      },
...
Package.prototype.generateManifest = function (tempPackagePath, callback) {
  var self = this;
  ...
  callback(null, utils.addBOM(js2xml.serialize(pkg)));
};
