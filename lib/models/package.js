...
Package.prototype.addContentDefinition = function (filePath, originalFullPath) {
  ...
  var baseLocation = parts[0];

  var dataStorePath = null;
  if (baseLocation === 'LocalContent') {
    filePath = path.join.apply(path, parts.slice(1, parts.length));
    var hash = uuid.v4().replace(/-/g, '');
    dataStorePath = path.join(baseLocation, hash).replace(/\\/g, '/');
  } else {
    dataStorePath = filePath.replace(/\\/g, '/');
  }
...
Package.prototype.addFileDefinition = function (layout, filePath, originalFullPath) {
  ...
  var contentDefinition = this.addContentDefinition(path.join('LocalContent', filePath), originalFullPath);
...
Package.prototype.generateManifest = function (tempPackagePath) {
  var self = this;
      ...
      if (self.layouts[layoutName] && self.layouts[layoutName].length > 0) {
        ...
        self.layouts[layoutName].forEach(function (fileDefinition) {
          ...
          pkg.PackageDefinition.PackageLayouts.LayoutDefinition.LayoutDescription.FileDefinition.push({
            FilePath: ('/' + fileDefinition.filePath).replace(/\//g, '\\'),
            FileDescription: {
