...
Package.prototype.addFileDefinition = function (layout, filePath, originalFullPath) {
  ...
  this.addContentDefinition(path.join('LocalContent', layout, filePath), originalFullPath);
};
    ...
    Object.keys(self.layouts).forEach(function (layoutName) {
      ...
      if (self.layouts[layoutName] && self.layouts[layoutName].length > 0) {
        ...
        self.layouts[layoutName].forEach(function (fileDefinition) {
          var fileStat = fs.statSync(fileDefinition.originalFullPath);

          pkg.PackageDefinition.PackageLayouts.LayoutDefinition.FileDefinition.push({
            FilePath: '/' + fileDefinition.filePath,
            FileDescription: {
              DataContentReference: path.join('LocalContent', layoutName, fileDefinition.filePath),
