...
exports.createPackageFile = function (outputRoot, tempPackagePath, packageName, callback) {
  utils.getFilesDirectory(tempPackagePath, function (err, files) {
    ...
    files.forEach(function (file) {
      ...
      if (!fs.lstatSync(fullFilePath).isDirectory()) {
        if (file.substr(0, 'LocalContent'.length) === 'LocalContent') {
          contentTypes.addOverride(('/' + file).replace(/\\/g, '/'), 'application/octet-stream');
        }

        var isBinary = isBinaryFile(fullFilePath);
        if (isBinary) {
          var content = fs.readFileSync(fullFilePath, { encoding: 'binary' });
          zip.file(file, content.toString('binary'), { binary: true });
        } else {
          var content = fs.readFileSync(fullFilePath, { encoding: 'base64' });
    ...
    var contentTypesContent = utils.addBOM(contentTypes.generate().toString()).toString('base64');
    fs.writeFileSync(path.join(tempPackagePath, '[Content_Types].xml'), contentTypesContent, { encoding: 'base64' });
    zip.file('[Content_Types].xml', contentTypesContent, { base64: true });

    // Add .rels file
    utils.mkdir(path.join(tempPackagePath, '_rels/'));

    var relsContent = utils.addBOM(rels.generate().toString()).toString('base64');
    fs.writeFileSync(path.join(tempPackagePath, '_rels/.rels'), relsContent, { encoding: 'base64' });
    zip.file('_rels/.rels', relsContent, { base64: true });

    // Create .cspkg
    var data = zip.generate({ base64: false, compression: 'STORE' });

    var packagePath = path.join(outputRoot, packageName);
    fs.writeFileSync(packagePath, data, { encoding: 'binary' });
