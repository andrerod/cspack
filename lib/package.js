...
_.extend(Package.prototype, {
  ...
  * @param {string}   name               The name of the file when in a layout.
  * @param {string}   originalFullPath   The file to add to the store.
  * @param {function} callback           The callback function.
  */
  addContentDefinition: function (store, name, originalFullPath, callback) {
    var self = this;
    ...
    function processFile(err, sha) {
      ...
      if (contentDefinition) {
        ...
        var dataStorePath = self.normalizeDataStorePath(path.join(store, name || self.generateDataStoreHash()));
        contentDefinition = {
          Name: dataStorePath,
          ContentDescription: {
  ...
  addFileDefinition: function (layout, rolePath, fileFullPath, callback) {
    ...
    self.addContentDefinition('LocalContent', null, fileFullPath, function (err, contentDefinition) {
