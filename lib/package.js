...
_.extend(Package.prototype, {
  ...
  addContentDefinition: function (store, filePath, originalFullPath, callback) {
    ...
    function addLengthInBytes (contentDefinition) {
      contentDefinition.ContentDescription.LengthInBytes = fs.statSync(path.join(self.temporaryPackagePath, 'LocalContent', contentDefinition.Name)).size;
    ...
    if (_.isObject(filePath)) {
      ...
      contentDefinition = this.contents.filter(function (content) {
        return content.Name === realFilePath;
      })[0];
      ...
      if (originalFullPath && originalFullPath.substr(0, self.temporaryPackagePath.length) !== self.temporaryPackagePath) {
        utils.copyFile(
          originalFullPath,
          path.join(self.temporaryPackagePath, dataStorePath),
