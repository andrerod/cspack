...
_.extend(Package.prototype, {
  ...
  */
  addContentDefinition: function (store, name, fileFullPath, callback) {
    ...
    self.dataStore.addContentFromFile(dataStorePath, fileFullPath, function (err) {
  ...
  * @param {string} fileFullPath The file's full path.
  * @param {function(err, fileDefinition)} callback The callback function.
  */
  addFileDefinition: function (layoutName, rolePath, fileFullPath, callback) {
    var self = this;
    ...
    self.addContentDefinition(constants.PackagePaths.LocalContent, null, fileFullPath, function (err, contentDefinition) {
      if (err) { return callback(err); }
