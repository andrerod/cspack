...
Package.prototype.generateManifest = function (tempPackagePath, packageFilename, callback) {
  ...
  // TODO: remove all this logic and move it to the actual methods
  ...
  if (self.contents && self.contents.length > 0) {
    ...
    self.contents.forEach(function (content) {
      pkg.PackageContents.ContentDefinition.push({
  ...
  if (self.layouts) {
    ...
    Object.keys(self.layouts).forEach(function (layoutName) {
      pkg.PackageLayouts.LayoutDefinition = {
        Name: layoutName
      };

      if (self.layouts[layoutName] && self.layouts[layoutName].length > 0) {
        pkg.PackageLayouts.LayoutDefinition.LayoutDescription = {
          FileDefinition: []
        };
        ...
        self.layouts[layoutName].forEach(function (fileDefinition) {
          ...
          pkg.PackageLayouts.LayoutDefinition.LayoutDescription.FileDefinition.push({
  ...
  var packageXml = Handlebars.compile(
      fs.readFileSync(path.join(__dirname, 'templates/package.xml.handlebars')).toString()
    )(pkg);

  fs.writeFileSync(path.join(tempPackagePath, packageFilename), utils.addBOM(packageXml));
  callback(null);
};
