...
_.extend(CsPack.prototype, {
          ...
          if (err) {
            ...
            async.eachSeries(files, addApplicationFile, function () {
              ...
              case constants.RoleTypes.Worker:
                ...
                self.processTemplate(layoutDefinitionName, 'LocalContent', 'Roles/WorkerRole/Cloud.uar', 'Roles/WorkerRole/Cloud.uar.csman.handlebars', roleData, false, function () {
                  self.processTemplate(layoutDefinitionName, 'LocalContent', 'Roles/WorkerRole/RoleModel.xml', 'Roles/WorkerRole/RoleModel.xml.handlebars', roleData, false, function () {
                    self.processTemplate(layoutDefinitionName, 'LocalContent', 'Roles/WorkerRole/RuntimeSetup.Manifest', 'Roles/WorkerRole/RuntimeSetup.Manifest.handlebars', roleData, false, function () {
  ...
  processTemplate: function (layout, store, outputFile, templateFile, templateData, addBom, cb) {
    ...
    if (layout) {
      self.package.addFileDefinition(layout, dataStorePath, fullPath, deleteIfRequired);
    } else {
      self.package.addContentDefinition(store, dataStorePath, fullPath, deleteIfRequired);
    }
  ...
  addRequiredFeaturesToPackage: function (callback) {
    ...
    function addRequiredRoleFeatures (roleName, cb) {
      ...
      self.processTemplate(null, 'NamedStreams', streamName, 'RequiredFeatures/WorkerRole/1.0.handlebars', features, false, cb);
    }
  ...
  addSupportedOsToPackage: function (callback) {
    ...
    function addSupportedRoleOs (roleName, cb) {
      ...
      self.processTemplate(null, 'NamedStreams', streamName, 'SupportedOsFamilies/WorkerRole/1.0.handlebars', oSes, false, cb);
    }
  ...
  addSupportDataToPackage: function (callback) {
    ...
    function addSupportRoleData (roleName, cb) {
      ...
      self.processTemplate(null, 'NamedStreams', streamName, 'SupportData/WorkerRole/1.0.handlebars', supportData, false, cb);
    }
