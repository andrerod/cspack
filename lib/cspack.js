...
_.extend(CsPack.prototype, {
  ...
  getApplicationOutputPath: function (role, roleOutputPath) {
    return path.join(roleOutputPath, 'approot');
  },
      ...
      utils.getFilesDirectory(path.join(self.inputDirectory, roleName), function (err, files) {
        if (err) {
          ...
          var processWorkerRoleTemplate = function (templateFile, roleData) {
            ...
            var fullName = path.join(self.tempPackagePath, 'LocalContent/Roles', roleName, templateFile);
  ...
  addServiceDefinitions: function (callback) {
    ...
    var processTemplate = function (templateFile) {
      ...
      fs.writeFileSync(path.join(self.tempPackagePath, templateFile), output);
  ...
  addNamedStreams: function (callback) {
    ...
    Object.keys(namedStreams).forEach(function (namedStream) {
      ...
      fs.writeFileSync(namedStreamPath, namedStreams[namedStream].toString());
  ...
  createPackage: function (callback) {
    ...
    if (self.package.contents) {
      self.package.contents.forEach(function (content) {
        if (content.originalFullPath.substr(0, self.tempPackagePath.length) !== self.tempPackagePath) {
          filesToCopy.push({
            from: content.originalFullPath,
            to: path.join(self.tempPackagePath, 'LocalContent', content.filePath)
          });
        }
      });
    }

    if (self.package.layouts) {
      Object.keys(self.package.layouts).forEach(function (layoutName) {
        if (self.package.layouts[layoutName]) {
          self.package.layouts[layoutName].forEach(function (content) {
            if (content.originalFullPath.substr(0, self.tempPackagePath.length) !== self.tempPackagePath) {
              filesToCopy.push({
                from: content.originalFullPath,
                to: path.join(self.tempPackagePath, 'LocalContent', layoutName, content.filePath)
              });
            }
          });
        }
    ...
    copyFiles(filesToCopy, function () {
      ...
      fs.writeFileSync(path.join(self.tempPackagePath, constants.PackageFile), self.package.generateManifest());
