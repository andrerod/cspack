...
_.extend(CsPack.prototype, {
  ...
  processTemplate: function (layout, store, templateFile, templateData, addBom, cb) {
    ...
    var dataStorePath;
    if (layout) {
      dataStorePath = self.package.getNewDataStoreHash();
    } else {
      dataStorePath = path.basename(templateFile, '.handlebars');
    }

    var fullPath = path.join(self.temporaryPackagePath, store, dataStorePath);
  ...
  addNamedStreams: function (callback) {
    ...
    function addStream(namedStream, cb) {
      ...
      self.package.addContentDefinition('NamedStreams', namedStream, namedStreamPath, cb);
    }

    async.each(Object.keys(namedStreams), addStream, callback);
  },
