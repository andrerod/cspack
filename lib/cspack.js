...
_.extend(CsPack.prototype, {
  ...
  addLocalContents: function (callback) {
    ...
    function createRole(roleName, cb) {
      ...
      function addBaseFiles (files, done) {
        if (files.length > 0) {
      ...
      function addUserFiles (files, done) {
        if (files.length > 0) {
          var file = files.shift();
      ...
      addBaseFiles(baseFiles, function () {
          ...
          if (err) {
            ...
            addUserFiles(files, function () {
              ...
              case constants.RoleTypes.Worker:
                ...
                self.processTemplate(layoutDefinitionName, 'LocalContent', 'Roles/WorkerRole/Cloud.uar.csman.handlebars', roleData, false, function () {
                  self.processTemplate(layoutDefinitionName, 'LocalContent', 'Roles/WorkerRole/RoleModel.xml.handlebars', roleData, false, function () {
                    self.processTemplate(layoutDefinitionName, 'LocalContent', 'Roles/WorkerRole/RuntimeSetup.Manifest.handlebars', roleData, false, function () {
  ...
  processTemplate: function (layout, store, templateFile, templateData, addBom, cb) {
    var self = this;
    ...
    var fullPath = path.join(self.temporaryPackagePath, store, path.basename(templateFile, '.handlebars'));
    utils.mkdir(path.dirname(fullPath));
    fs.writeFileSync(fullPath, output);

    if (layout) {
      self.package.addFileDefinition(layout, path.basename(templateFile, '.handlebars'), fullPath, deleteIfRequired);
    } else {
      self.package.addContentDefinition(store, path.basename(templateFile, '.handlebars'), fullPath, deleteIfRequired);
    }
  ...
  addServiceDefinitions: function (callback) {
    ...
    self.processTemplate(null, 'ServiceDefinition', 'ServiceDefinition.csdef.handlebars', self.serviceDefinitionModel, true, function () {
      self.processTemplate(null, 'ServiceDefinition', 'ServiceDefinition.rd.handlebars', self.serviceDefinitionModel, false, function () {
        self.processTemplate(null, 'ServiceDefinition', 'ServiceDefinition.rdsc.handlebars', self.serviceDefinitionModel, true, function () {
  ...
  addNamedStreams: function (callback) {
    ...
    function addNamedStreams(namedStreams, cb) {
      if (namedStreams.length > 0) {
        var namedStream = namedStreams.shift();
        ...
        self.package.addContentDefinition('NamedStreams', namedStream, namedStreamPath, function () {
          addNamedStreams(namedStreams, cb);
        });
      } else {
        cb();
      }
    ...
    addNamedStreams(namedStreams, callback);
  },
