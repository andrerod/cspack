...
_.extend(CsPack.prototype, {
      ...
      utils.getFilesDirectory(path.join(self.inputDirectory, roleName), function (err, files) {
        if (err) {
          ...
          case constants.RoleTypes.Worker:
            processWorkerRoleTemplate('Cloud.uar.csman', { name: roleName }, false);
            processWorkerRoleTemplate('RoleModel.xml', { name: roleName }, false);
            processWorkerRoleTemplate('RuntimeSetup.Manifest', { name: roleName }, false);
  ...
  addServiceDefinitions: function (callback) {
    ...
    var processTemplate = function (templateFile, addBom) {
      ...
      fs.writeFileSync(path.join(self.tempPackagePath, 'ServiceDefinition', templateFile), output);

      self.package.addContentDefinition('ServiceDefinition', templateFile, path.join(self.tempPackagePath, 'ServiceDefinition', templateFile));
    };

    processTemplate('ServiceDefinition.csdef', true);
    processTemplate('ServiceDefinition.rd', false);
    processTemplate('ServiceDefinition.rdsc', true);
  ...
  createPackage: function (callback) {
    ...
    copyFiles(filesToCopy, function () {
      ...
      self.package.generateManifest(self.tempPackagePath, function (err, manifest) {
        fs.writeFileSync(path.join(self.tempPackagePath, constants.PackageFile), manifest);
