...
_.extend(CsPack.prototype, {
      ...
      utils.getFilesDirectory(path.join(self.inputDirectory, roleName), function (err, files) {
        if (err) {
          ...
          case constants.RoleTypes.Worker:
            processWorkerRoleTemplate('Cloud.uar.csman', { name: roleName });
            processWorkerRoleTemplate('RoleModel.xml', { name: roleName });
            processWorkerRoleTemplate('RuntimeSetup.Manifest', { name: roleName });
  ...
  addServiceDefinitions: function (callback) {
    ...
    processTemplate('ServiceDefinition.rd');
