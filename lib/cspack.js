...
_.extend(CsPack.prototype, {
  ...
  */
  initializePackage: function (callback) {
    ...
    // TODO: move the cache path to somewhere in the user home folder
    self.sdkPackage = new SDKPackage({ cachePath: path.join(__dirname, 'cache' ) });
  ...
  addRoleSDKFiles: function(roleName, layoutDefinitionName, callback) {
    ...
    self.sdkPackage.getFiles(function (err, roleFiles) {
      ...
      async.eachSeries(roleFiles, addRoleFile, function (err) {
        ...
        self.processTemplate(layoutDefinitionName, constants.PackagePaths.LocalContent, 'Cloud.uar.csman', 'Roles/Cloud.uar.csman.handlebars', roleData, false, function () {
